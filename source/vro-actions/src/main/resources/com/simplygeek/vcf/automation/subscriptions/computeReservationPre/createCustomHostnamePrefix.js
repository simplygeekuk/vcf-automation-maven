...
(function (inputProperties) {
    ...
    ).Logger())("Action", "createCustomHostnamePrefix");
    ...
    var customNamingStartCounter =
        customNamingConfigService.getStartCounter() - 1;
    var customNamingIncrementStep =
        customNamingConfigService.getIncrementStep();
    ...
    log.info(
        "Adding hostname prefix '" +
            hostnamePrefix +
            " to custom naming profile '" +
            customNamingProfile.name +
            "'"
    );
    if (
        customNamingService.checkCustomNamingPrefixExists(
        ...
        try {
            ...
            throw new Error(
                "Unexpected error creating custom naming prefix: " + e
            );
        } finally {
