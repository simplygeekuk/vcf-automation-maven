...
(function (restHostName) {
    if (!restHostName || typeof restHostName !== "string") {
        throw new ReferenceError(
            "restHostName is required and must be of type 'string'"
        );
    }

    var log = new (System.getModule(
        "com.simplygeek.vcf.orchestrator.logging"
    ).Logger())("Action", "getRestHost");
    var restHost;

    log.debug("Getting Rest Host with name: " + restHostName);
    var allRestHosts = RESTHostManager.getHosts().map(function (hostId) {
        return RESTHostManager.getHost(hostId);
    });
    var restHostsFound = allRestHosts.filter(function (host) {
    ...
    if (restHostsFound.length > 1) {
        ...
        throw new Error(
            "No Rest Host found with the name '" + restHostName + "'"
        );
    }
