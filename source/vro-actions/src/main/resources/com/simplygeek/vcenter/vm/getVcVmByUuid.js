 ...
 */
(function (vcVmInstanceUuid, vCenterSdkConnection) {
    ...
    if (
        ...
        throw new ReferenceError(
            "vCenterSdkConnection must be of type 'VC:SdkConnection'"
        );
    }

    var log = new (System.getModule(
        "com.simplygeek.vcf.orchestrator.logging"
    ).Logger())("Action", "getVcVmByUuid");
    var vcVm;
    var vcVms;

    log.debug("Get Virtual Machine with UUID '" + vcVmInstanceUuid + "'");
    var xpath =
        "xpath:config/instanceUuid[starts-with(.,'" + vcVmInstanceUuid + "')]";
    ...
    if (vcVms.length > 1) {
        throw new Error(
            "More than one Virtual Machine was found with UUID '" +
                vcVmInstanceUuid +
                "'"
        );
        ...
        throw new Error(
            "No Virtual Machine found with UUID '" + vcVmInstanceUuid + "'"
        );
    }
